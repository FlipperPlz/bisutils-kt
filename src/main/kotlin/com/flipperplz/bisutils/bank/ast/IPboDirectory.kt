...
interface IPboDirectory : IFamilyParent, IPboVFSEntry, Cloneable {
    ...
    val directories: List<IPboDirectory> get() = familyChildren.filterIsInstance<IPboDirectory>() ?: emptyList()
    val entries: List<IPboEntry> get() = familyChildren.filterIsInstance<IPboEntry>() ?: emptyList()

    override fun calculateBinaryLength(options: PboOptions?): Long =
        familyChildren.sumOf { it.calculateBinaryLength(options) } ?: 0

    override fun read(buffer: ByteBuffer, options: PboOptions): Boolean =
        throw Exception("Not Supported!")

    override fun isValid(options: IOptions?): Boolean {
        familyChildren.forEach { if (!it.isValid(options)) return false }
        return true
    }
